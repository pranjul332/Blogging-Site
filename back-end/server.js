...
// Create post route
app.post('/Create', verifyToken, upload.fields([{ name: 'photo' }, { name: 'video' }]), async (req, res) => {
    ...
    try {
        ...
        const post = new Post({ title, discription, photo, video, user: req.user.id });
...
app.post('/registration', async (req, res) => {
    try {
        ...
        if (existingUser) {
            return res.json({ success: 'user', message: 'Username already registered' });
        } else if (existingEmail) {
            return res.json({ success: 'email', message: 'Email already registered' });
        }
...
app.post('/profile-picture', verifyToken, upload.single('profilePicture'), async (req, res) => {
    try {
...
app.post('/post/:id/like', verifyToken, async (req, res) => {
    try {
        ...
        if (hasLiked) {
            ...
            post.liked = post.liked.filter(id => id.toString() !== userId.toString());
        } else {
